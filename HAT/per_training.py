...
import os
import torch
from exp.exp_pre_train import Exp_Pre_Train
import random
import numpy as np
...
if __name__ == '__main__':
    ...
    parser.add_argument('--keep_ratio', type=float, default=0.25, help='keep_ratio')
    ...
    # basic config
    parser.add_argument('--task_name', type=str, required=False, default='per_training',
                        help='task name, options:[per_training, long_term_forecast, short_term_forecast,'
    ...
    if args.is_training:
        ...
        print('>>>>>>>start Pre_Train : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
        exp.train(setting)
